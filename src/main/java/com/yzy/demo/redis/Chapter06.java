...
public class Chapter06 {
    ...
    public void run() throws InterruptedException, IOException {
        Jedis conn = new Jedis("39.96.0.182");
        conn.select(15);
//        testAddUpdateContact(conn);
//        testAddressAutocomplete(conn);
...
//        testMultiRecipientMessaging(conn);
    ...
    public class CopyLogsThread  extends Thread
    {
        ...
        this.conn = new Jedis("39.96.0.182");
        this.conn.select(15);
        this.path = path;
    ...
    public void testAddUpdateContact(Jedis conn){
        ...
        System.out.println("let's add few contacts..");
        IntStream.range(0,10)
                .forEach(i->addUpdateContact(conn,"young","contact-"
                        + (int)Math.floor(i/3) + "-" + i));

        System.out.println("young's current recnetly contacted contacts:");
        List<String> contacts = conn.lrange("recent:young",0,100);
        contacts.forEach(contact-> System.out.println(contact));
        assert contacts.size() >= 10;

        System.out.println("let's pull one of the order up to the front");
        addUpdateContact(conn,"young","contact-1-4");
        contacts = conn.lrange("recent:young",0,2);
        System.out.println("new top-3 contacts:");
        contacts.forEach(contact-> System.out.println(contact));
        assert contacts.get(0).equals("contact-1-4");

        System.out.println("let's remove a contact..");
        removeContact(conn,"young","contact-2-6");
        contacts = conn.lrange("recent:young",0,-1);
        System.out.println("after remove contact-2-6..");
        contacts.forEach(contact-> System.out.println(contact));
        assert contacts.size() >= 9;

        System.out.println("let's finally autocomplete..");
        //获取指定前缀的联系人匹配结果
        contacts = fetchAutocompleteList(conn,"young","c");
        contacts.forEach(contact-> System.out.println(contact));
    }
    ...
    public void addUpdateContact(Jedis conn,String user,String contact){
        // 用户最近联系人list列表键名
        String contactList = "recent:"+user;
        // 开始添加最近联系人的事务操作
        Transaction transaction = conn.multi();
        // 如果联系人存在list中，删除
        transaction.lrem(contactList,0,contact);
        // 把联系人加入list最新的位置
        transaction.lpush(contactList,contact);
        // 把list多余的数据删除
        transaction.ltrim(contactList,0,99);
        transaction.exec();
    }

    public void testAddressAutocomplete(Jedis conn){
        conn.del("members:young");
        System.out.println("---testAddressAutocomplete---");
        System.out.println("let's add some people to guild..");
        for (String name : new String[]{"jeff", "jenny", "jack", "jennifer"}){
            joinGuild(conn, "soccer", name);
        }
        System.out.println();
        System.out.println("now let's try to find users with names starting with 'je':");
        Set<String> r = autocompleteOnPrefix(conn, "soccer", "je");
        System.out.println(r);
        assert r.size() == 3;

        System.out.println("jeff just left to join a different guild...");
        leaveGuild(conn, "test", "jeff");
        r = autocompleteOnPrefix(conn, "young", "je");
        System.out.println(r);
        assert r.size() == 2;
        conn.del("members:young");
    }
    ...
    public String acquireLock(Jedis conn, String lockName, long acquireTimeout){
        ...
        while (now < end) {
            ...
            try {
                ...
                System.out.println("acquireLock fail");
    ...
    boolean releaseLock(Jedis conn, String lock, String identifier){
        while(true){
            ...
            throws InterruptedException
    {
        ...
        if(acquireLockWithTimeout(conn, "lock:testlock",
                1000, 50000) != null){
            System.out.println("Got it!");
        }
        System.out.println("Trying to get it again without releasing the first one...");
        String result = acquireLockWithTimeout(conn,
                "lock:testlock", 20000, 50000);
        if(null == result)
        {
        ...
        String lockId = acquireLockWithTimeout(conn, "lock:testlock",
                20000, 40000);
        assert lockId != null;
        System.out.println("Got it!");
        System.out.println("Releasing the lock...");
        assert releaseLock(conn, "lock:testlock", lockId);
        System.out.println("Released it...");
        System.out.println();

        System.out.println("Acquiring it again...");
        if (acquireLockWithTimeout(conn, "testlock",
                20000, 3000) != null){
            System.out.println("Got it!");
        }
        conn.del("lock:testlock");
    ...
    public String acquireFairSemaphore(Jedis conn, String semname,
            ...
            throws InterruptedException
    {
        ...
        for (int i = 0; i < 3; i++) {
            acquireFairSemaphore(conn, "testsem", 3, 50000) ;
        }
        System.out.println("Done!");
        ...
        String id = acquireFairSemaphore(conn, "testsem", 3, 50000);
        assert id != null;
        System.out.println("Got one!");
        System.out.println("Let's release it...");
        releaseFairSemaphore(conn, "testsem", id);
        System.out.println("Released!");
        System.out.println();
        System.out.println("And let's make sure we can get 3 more!");
        int semResult = 0;
        for (int i = 0; i < 3; i++) {
            if(acquireFairSemaphore(conn, "testsem", 3, 1000) != null){
                semResult++;
            }
        }
        System.out.println("We got them!" + semResult);
        conn.del("testsem", "testsem:owner", "testsem:counter");
    }
            ...
            extends Thread
    {
        ...
        public void run() {
            while (!quit){
                ...
                String locked = acquireLock(conn, identifier,10000);
                if (locked == null){
                    continue;
                }
                ...
                releaseLock(conn, "lock:"+identifier, locked);
            }
        }
    }
    ...
    public String sendMessage(Jedis conn, String chatId, String sender, String message) {
        String identifier = acquireLock(conn, "chat:" + chatId,50000);
        if (identifier == null){
